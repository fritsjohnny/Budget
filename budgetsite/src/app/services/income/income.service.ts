...
export class IncomeService {
  ...
  read(reference: string, justMyValues: boolean): Observable<Incomes[]> {
    return this.http
      .get<Incomes[]>(
        `${ApiUrls.incomes}/reference${justMyValues ? '2' : ''}/${reference}`
      )
      .pipe(
        map((obj) => obj),
        catchError((e) => this.messenger.errorHandler(e))
  ...
  updateValue(id: number, value: number): Observable<Incomes> {
    return this.http
      .put<Incomes>(`${ApiUrls.incomes}/AddValue/${id}?value=${value}`, null)
      .pipe(
        map((obj) => obj),
        catchError((e) => this.messenger.errorHandler(e))
      );
  ...
  orderByPreviousMonth(reference: string): Observable<any> {
    return this.http
      .post<any>(
        `${ApiUrls.incomes}/OrderByPreviousMonth?Reference=${reference}`,
        null
      )
      .pipe(
        map((obj) => obj),
